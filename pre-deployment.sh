...
make_atm_microservice_default_namespace() {
  ...
  # Set the namespace to atm-microservices
  echo "Setting namespace to atm-microservices..."
  kubectl config set-context --current --namespace=atm-microservices
...
expose_argocd() {
  # Prompt user for exposing method
  ...
  if [[ "$CHOICE" -eq 1 ]]; then
    # Assuming you have an Ingress controller and a values file to configure the ingress
    echo "Setting up ingress for Argo CD..."
    # You may want to provide your custom values file for ingress configuration
    helm upgrade argocd "$ARGOCD_CHART_DIR" --namespace atm-microservices -f ./argo-cd/values-ingress.yaml
    echo "Ingress for Argo CD has been set up. Check your ingress resource with:"
    echo "kubectl get ingress -n atm-microservices"
  elif [[ "$CHOICE" -eq 2 ]]; then

    # Port-forwarding to the Argo CD server service
    local port
    read -p "Enter the local port you want to use for port-forwarding (default is 8080): " port
    port=${port:-8080}  # Default to 8080 if no input is provided

    echo "Port-forwarding to Argo CD server on port $port..."
    kubectl port-forward svc/argocd-server -n atm-microservices "$port":443 &

    echo "Argo CD is now accessible at http://localhost:4040"
