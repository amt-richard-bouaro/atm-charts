...
expose_argocd() {
  ...
  if [[ "$CHOICE" -eq 1 ]]; then
    ...
    if kubectl get svc/argocd-server -n atm-microservices &> /dev/null; then
      SERVICE_NAME="argocd-server"
    elif kubectl get svc/argo-cd-argocd-server -n atm-microservices &> /dev/null; then
    ...
    kubectl port-forward svc/"$SERVICE_NAME" -n atm-microservices "$port":443 &
